...
describe('Feature: ciclo de vida completo do pedido', () => {
  ...
  beforeEach(async () => {
    ...
    const client = await createUser(
      'lifecycle.client@taskgo.test',
      '52998224725',
      UserType.CLIENTE,
    );
    const otherClient = await createUser(
      'lifecycle.other-client@taskgo.test',
      '11144477735',
      UserType.CLIENTE,
    );
    const provider = await createProvider(
      'lifecycle.provider@taskgo.test',
      '39053344705',
    );
    const otherProvider = await createProvider(
      'lifecycle.other-provider@taskgo.test',
      '16899535009',
    );
    ...
    orderId = order.id;
    clientToken = await tokenFor(clientId);
    otherClientToken = await tokenFor(otherClientId);
    providerToken = await tokenFor(providerId);
    otherProviderToken = await tokenFor(otherProviderId);
    gateway.authorizeCardPayment.mockResolvedValue({
      orderId: 'or_lifecycle',
      chargeId: 'ch_lifecycle',
  ...
  describe('Scenario: aceite do pedido', () => {
    it('Given pedido aguardando aprovação, When prestador responsável aceita, Then aguarda pagamento', async () => {
      const response = await post(
        `/orders/${orderId}/provider/${providerId}/confirm`,
        providerToken,
      ).expect(201);
    ...
    it('Given pedido de outro prestador, When tentar aceitar, Then retorna 403', async () => {
      await post(
        `/orders/${orderId}/provider/${otherProviderId}/confirm`,
        otherProviderToken,
      ).expect(403);
      expect(
        (await prisma.order.findUniqueOrThrow({ where: { id: orderId } }))
          .status,
      ).toBe(OrderStatus.AGUARDANDO_APROVACAO);
    });
  ...
  describe('Scenario: finalização pelo prestador', () => {
    ...
    it('Given preço alterado sem justificativa, When finalizar, Then retorna 400 e não muda status', async () => {
      await setStatus(OrderStatus.EM_ANDAMENTO);
      await patch(`/orders/${orderId}/finish`, providerToken, {
        ...finishPayload(),
        finalPrice: 150,
  ...
  describe('Scenario: confirmação pelo cliente', () => {
    ...
    it('Given serviço finalizado, When outro cliente confirmar, Then retorna 403', async () => {
      await prepareFinishedCardOrder();
      await patch(`/orders/${orderId}/confirm`, otherClientToken, {}).expect(
        403,
      );
    });
  ...
  describe('Scenario: avaliação e reputação', () => {
    it('Given pedido não concluído, When avaliar, Then retorna 400', async () => {
      await post(`/orders/${orderId}/review`, clientToken, {
        rating: 5,
      }).expect(400);
    });
    ...
    it('Given pedido concluído, When dono avalia com tag ativa, Then atualiza reputação', async () => {
      ...
      const tag = await prisma.reviewTag.findFirstOrThrow({
        where: { isActive: true },
      });
      ...
      expect(response.body.tags).toEqual([
        expect.objectContaining({ id: tag.id.toString() }),
      ]);
      expect(response.body.provider).toEqual(
        expect.objectContaining({ ratingAvg: 5, ratingCount: 1 }),
      );
    });
    ...
    it('Given pedido concluído, When outro cliente avalia, Then retorna 403', async () => {
      await prepareConcludedOrder();
      await post(`/orders/${orderId}/review`, otherClientToken, {
        rating: 5,
      }).expect(403);
    });
  });
  ...
  async function prepareConcludedOrder() {
    await prisma.order.update({
      where: { id: orderId },
      data: { status: OrderStatus.CONCLUIDO },
    });
  }
  ...
  async function tokenFor(id: bigint) {
    return (await tokens.createToken(id)).access_token;
  }
  function createUser(email: string, cpf: string, type: UserType) {
  ...
  function createProvider(email: string, cpf: string) {
