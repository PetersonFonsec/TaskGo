...
describe('Feature: pagamento de pedidos', () => {
  ...
  beforeEach(async () => {
    ...
    const client = await prisma.user.create({
      data: {
        name: 'Cliente Pagamento E2E',
        email: 'payment.client@taskgo.test',
    ...
    const otherClient = await prisma.user.create({
      data: {
        name: 'Outro Cliente E2E',
        email: 'payment.other@taskgo.test',
        passwordHash: 'not-used',
        cpf: '11144477735',
        type: UserType.CLIENTE,
      },
      ...
      data: {
        ...
        user: {
          create: {
            ...
            email: 'payment.provider@taskgo.test',
            passwordHash: 'not-used',
            cpf: '39053344705',
  ...
  describe('Scenario: validação do payload', () => {
    it.each([
      ...
      [
        'cartão inválido',
        { method: 'CARTAO', card: { ...validCard(), cvv: '1' } },
        400,
      ],
    ])(
      'Given %s, When iniciar pagamento, Then rejeita a requisição',
      async (_, payload, status) => {
  ...
  describe('Scenario: pedido precisa existir e pertencer ao cliente', () => {
    ...
    it('Given outro cliente, When pagar o pedido, Then retorna 403', async () => {
      await postPayment(orderId, otherClientToken, { method: 'PIX' }).expect(
        403,
      );
    });
  });
  ...
  describe('Scenario: estado do pedido', () => {
    it('Given pedido fora de AGENDADO/AGUARDANDO_PAGAMENTO, When pagar, Then retorna 400', async () => {
      await prisma.order.update({
        where: { id: orderId },
        data: { status: OrderStatus.EM_ANDAMENTO },
      });
  ...
  describe('Scenario: pagamento PIX', () => {
    ...
    it('Given PIX pendente existente, When repetir, Then reutiliza sem duplicar cobrança', async () => {
      const first = await postPayment(orderId, clientToken, {
        method: 'PIX',
      }).expect(201);
      const second = await postPayment(orderId, clientToken, {
        method: 'PIX',
      }).expect(201);
  ...
  async function cleanScenario() {
    const users = await prisma.user.findMany({
      where: { email: { startsWith: 'payment.' } },
      select: { id: true },
    });
