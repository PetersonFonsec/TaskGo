  ...
  async execute({ orderId, clientId, payload }: CreateOrderReviewCommand) {
    ...
    if (order.clientId !== clientId)
      throw new ForbiddenException(
        'Apenas o cliente deste pedido pode avaliar o prestador',
      );
    if (order.service.providerId === clientId)
      throw new ForbiddenException('O prestador não pode avaliar a si mesmo');
    if (order.status !== OrderStatus.CONCLUIDO)
      throw new BadRequestException(
        'Somente pedidos concluídos podem ser avaliados',
      );
    if (order.review)
      ...
      return await this.prisma.$transaction(
        async (prisma) => {
          ...
          if (activeTags.length !== tagIds.length) {
            throw new BadRequestException(
              'Uma ou mais tags são inválidas ou estão inativas',
            );
          }
