...
export class FinishServicePage implements OnInit {
  ...
  estimatedPrice = computed(
    () => this.order()?.payment?.estimatedAmount ?? this.order()?.service.estimatedPrice ?? 0,
  );
    ...
    this.#route.paramMap
      ...
      .subscribe({
        next: (order) => {
          ...
          this.order.set(order);
          this.form.controls.finalPrice.setValue(
            order.payment?.estimatedAmount ?? order.service.estimatedPrice,
          );
        },
  ...
  submit(): void {
    ...
    this.#orders
      .finishOrder(this.order()!.id, {
        finalPrice: this.estimatedPrice(),
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) =>
          this.error.set(
            error.error?.message || 'Não foi possível enviar a conclusão. Tente novamente.',
          ),
      });
  }
