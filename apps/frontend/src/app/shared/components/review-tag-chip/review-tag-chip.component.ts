...
import { ReviewTag } from '@shared/service/order/order.model';

@Component({
  selector: 'app-review-tag-chip',
  template: `<button type="button" [class.selected]="selected()" [attr.aria-pressed]="selected()" (click)="toggle.emit()"><span>{{ selected() ? '✓' : '+' }}</span>{{ tag().name }}</button>`,
